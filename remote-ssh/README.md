...
### 1.9 NOPASSWD sudo (远程全自动化的前提) ⭐

Mac 上用 `ssh ubuntu "sudo ..."` 或让 Claude/CI 远程自动部署时,sudo 弹密码会卡死(SSH 默认无 tty,无法交互输入)。一次性配 NOPASSWD 后,所有远程 sudo 都直接执行。

#### 配置(在 Ubuntu 上,需输一次密码)

```bash
echo "zhang ALL=(ALL) NOPASSWD: ALL" | sudo EDITOR='tee' visudo -f /etc/sudoers.d/zhang-nopasswd
sudo chmod 0440 /etc/sudoers.d/zhang-nopasswd
```

> ⚠️ 必须用 `visudo`,它做语法检查,改坏不会落盘。直接 `nano /etc/sudoers` 改坏后所有 sudo 都报错。

#### Mac 上一行远程配置(替代上面)

```bash
ssh -t ubuntu 'echo "zhang ALL=(ALL) NOPASSWD: ALL" | sudo EDITOR="tee" visudo -f /etc/sudoers.d/zhang-nopasswd && sudo chmod 0440 /etc/sudoers.d/zhang-nopasswd'
```
...
```bash
# Mac 上
ssh ubuntu "sudo -n whoami"           # 应返回: root
ssh ubuntu "sudo -n cat /etc/sudoers.d/zhang-nopasswd"
# 应返回: zhang ALL=(ALL) NOPASSWD: ALL
```

`sudo -n` 是 `--non-interactive`,要密码就直接报错而非卡住,适合脚本使用。

#### 安全前提(必须三项全做完才能 NOPASSWD)

```bash
# 1. SSH 已禁用密码登录
sudo grep -E "^(PasswordAuthentication)" /etc/ssh/sshd_config
# 期望: PasswordAuthentication no

# 2. Mac 私钥权限正确
ls -l ~/.ssh/id_ed25519       # 期望: -rw-------

# 3. 私钥有备份
ls -l /Volumes/T2/_AA/id_ed25519.backup 2>/dev/null
# 没有 → cp ~/.ssh/id_ed25519 /Volumes/T2/_AA/id_ed25519.backup && chmod 600 ...
```

#### 救命预案 (sudoers 语法弄坏)

```bash
# 方式 1: pkexec 不依赖 sudoers
pkexec rm /etc/sudoers.d/zhang-nopasswd

# 方式 2: 重启进 GRUB → Advanced → recovery mode → root shell

# 方式 3: 物理 Ctrl+Alt+F2 切 tty 用别的有 sudo 的账号
```

#### 取消 NOPASSWD

```bash
sudo rm /etc/sudoers.d/zhang-nopasswd
```
...
| 远程 `ssh ubuntu "sudo ..."` 卡死 | 没配 NOPASSWD, 见 §1.9 |
...
| v1.1 | 2026-05-18 | 新增 §1.9 NOPASSWD sudo (远程全自动化前提) + 安全前提清单 + 救命预案; 故障排查表加 "ssh sudo 卡死" 行 |
